...
async def show_favorite_2(message: types.Message):
        ...
        for ap in aps:
            ...
            for file in photo_files[1:]:
                with open(file, "rb") as f:
                    photo = types.InputFile(io.BytesIO(f.read()), filename=file)
                    media.append(types.InputMediaPhoto(media=photo, caption=caption))
            text_message = await message.bot.send_media_group(chat_id=message.chat.id,
                                                              media=media)
            if language in ['EN', 'IN']:
...
def register(dp: Dispatcher):
    ...
    dp.register_message_handler(show_favorite_2, commands='favorites', state="*")
